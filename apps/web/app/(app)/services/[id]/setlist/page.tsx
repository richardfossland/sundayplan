  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <SectionTitle eyebrow={t("services.setlist")}>{setlist.name}</SectionTitle>
          <p className="mt-1 text-sm text-ink-500">
            {date} · {t("services.songsCount", { count: setlist.songs.length })}
          </p>
        </div>
      ...
      {setlist.songs.length === 0 ? (
        <p className="py-12 text-center text-sm text-ink-500">
          {t("services.setlistEmpty.before")}{" "}
          <strong className="text-ink-300">{t("services.kind.song")}</strong>{" "}
          {t("services.setlistEmpty.after")}
        </p>
      ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-ink-400">
                    {s.key ? <Badge tone="gold">{t("services.songKey", { key: s.key })}</Badge> : null}
                    {s.tempo_bpm ? <Badge tone="info">{t("services.songBpm", { bpm: s.tempo_bpm })}</Badge> : null}
                    {s.themes.map((t) => (
