...
export function VolunteerAvailabilityForm({ token, blockouts }: { token: string; blockouts: Blockout[] }) {
    ...
    <div className="space-y-5">
      {blockouts.length > 0 ? (
        <div className="rounded-xl border border-white/[0.07] bg-ink-900/60 px-4 py-3">
          <p className="mb-2 text-xs font-medium uppercase tracking-wider text-ink-500">Your blockouts</p>
      ...
      <form action={formAction} className="space-y-4 rounded-xl border border-white/[0.07] bg-ink-900/60 px-4 py-4">
        ...
        <div>
          ...
          <input name="reason" placeholder="e.g. holiday" className={input} />
        </div>
        {state.error ? <p className="text-xs text-[color:var(--color-danger)]">{state.error}</p> : null}
        {state.ok ? <p className="text-xs text-[color:var(--color-success)]">Added — thanks!</p> : null}
        <button
          type="submit"
          disabled={pending}
