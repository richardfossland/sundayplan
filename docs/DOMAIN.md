...
- [~] Phase 1.3 — Auth — magic-link JWT core (`@sundayplan/auth`, 11 tests) + `0003` volunteer RLS + issue/respond Edge Functions (verified end-to-end) + **planner auth foundation** (`@supabase/ssr` client/server/middleware, route-group `(app)` shell vs `(auth)`, email sign-in/sign-up, middleware gating, sign-out + user email in TopBar; verified: unauthed→/sign-in 307, GoTrue signup/signin). Onboarding wizard (create/join church) + OAuth providers + real-data wiring still pending
