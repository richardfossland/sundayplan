...
export function VolunteerSwap({
  ...
  function ask(memberId: string) {
    ...
    startTransition(async () => {
      ...
      else setError(r.error ?? t("vol.swap.somethingWrong"));
    });
  }
  function handBack() {
    setError(null);
    ...
    return (
      <div className="rounded-xl border border-white/[0.07] bg-ink-900/60 px-5 py-8 text-center">
        <p className="text-xl font-semibold text-ink-50">
          {done === "proposed" ? t("vol.swap.proposedTitle") : t("vol.swap.openTitle")}
        </p>
        <p className="mt-2 text-sm text-ink-400">
          {done === "proposed" ? t("vol.swap.proposedBody") : t("vol.swap.openBody")}
        </p>
      </div>
    );
  }
          ...
          <ul className="space-y-1">
            {candidates.map((c) => (
              <li key={c.member_id} className="flex items-center justify-between gap-3 rounded-lg px-3 py-2 hover:bg-white/[0.04]">
                ...
                >
                  {t("vol.swap.ask", { name: c.name.split(" ")[0] })}
                </button>
              </li>
            ))}
          </ul>
